...
burst_fire_unmanned()
{
	...
	for( ;; )
	{
		...
		targets = GetAiSpeciesArray( "axis", "all" );
		for(i=0;i<targets.size;i++)
		{
			...
			else if(!BulletTracePassed(self.origin + (0,0,30), targets[i].origin + (0,0,30), false, undefined))
			{
				targets = array_remove(targets, targets[i]);
			}
		}
		...
		{
			self ClearTargetEntity();
			target = GetClosest(self.origin, targets);
			...
			while(IsDefined(target) && IsAlive(target) && BulletTracePassed(self.origin + (0,0,30), target.origin + (0,0,30), false, undefined) && GetClosest(self.origin, targets) == target)
			{
				shot = true;
				wait .1;
				self ShootTurret();
				if(target != self GetTurretTarget()) //had to put this check here because GetTurretTarget() doesnt seem to update right away
					break;
			}
		}
		else
...
DoShoot()
{
	...
	for( ;; )
	{
		if(!IsAlive(self GetTurretTarget()))
		{
			self ClearTargetEntity();
			wait .1;
			continue;
